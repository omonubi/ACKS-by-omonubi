...
| Encumbrance Alert | *enc_alert_toggle* | checkbox | If enabled, highlights the creature's encumbrance value any time it exceeds its *move_thresh1* (5 by default). This is to assist players with characters that have encumbrance-based abilities which require travelling light to take advantage of (e.g. tumbling, or a bladedancer's Graceful Fighting class power). |
